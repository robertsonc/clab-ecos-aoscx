...
renew_client_dhcp() {
    ...
    for client in $clients; do
        ...
        if docker exec "$container" sh -c "killall udhcpc 2>/dev/null; sleep 1; udhcpc -i eth1 -t 5 -T 3 -q 2>&1 | grep -q 'lease of' && ip route del default dev eth0 2>/dev/null" &>/dev/null; then
